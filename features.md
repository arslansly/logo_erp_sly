...
- **Kredi limiti — çok-firma graceful:** CLCARD'da tek `CREDITLIMIT` yok; bu kurulumda `DBSLIMIT1..7` (limit) + `DBSRISKCNTRL1..7` (risk kontrol) + `DUEDATELIMIT` var. Bu firmada **1176 carinin tamamında 0** ama ürün tüm firmalara satılacağı için özellik **eklendi**: `KrediLimiti = MAX(DBSLIMIT1..7)`, limit yoksa graceful **"tanımsız"** (firma 126'da böyle), limiti olan firmalarda gerçek limit/kalan/aşım gösterilir. Risk = (bakiye>0 ise bakiye) + açık sipariş; **limit aşıldı = risk > limit**.
...
- `Program.cs` — `SahaService` DI. `Services/CariService.cs` `GetByIdAsync` + `Models/Cari.cs` — cari detayına `AcikSiparis` + **`KrediLimiti` (MAX DBSLIMIT) + `RiskKontrolVar`** (sadece detayda; liste sorgusu değişmedi).
- **Kredi limiti (çok-firma):** `Models/SahaModels.cs` — `SahaRiskliCari.KrediLimiti` + `SahaOzet.LimitAsanSayisi`; `SahaService` — riskli müşteriye limit kolonu + özete **limit aşan müşteri sayacı** (verimli: önce `DBSLIMIT>0` süzülür, risk subquery yalnızca limitli cariler için çalışır; firma 126'da 0).
...
- `cari/cari_detay_screen.dart` + `cari_model.dart` — **"Risk durumu" kartı**: kredi limiti varsa **kullanım barı + kalan/aşım** (renk: <%80 yeşil, %80+ turuncu, aşım kırmızı + "Limit aşıldı" rozeti); limit yoksa açık sipariş + toplam risk + "Kredi limiti tanımsız" notu. `_hideBalance`'a saygılı. `Cari` getter'ları: `krediLimiti`/`riskKontrolVar`/`limitTanimli`/`kalanLimit`/`limitAsildi`/`limitKullanimOrani`/`toplamRisk`.
- **Kredi limiti UI:** `saha_model` — `SahaRiskliCari.limitAsildi` + `SahaOzet.limitAsanSayisi`; `saha_widgets` riskli tile'a **"Limit aşıldı"** rozeti; `saha_screen` koşullu **"N müşteri kredi limitini aştı"** banner'ı (yalnızca limit kullanan firmalarda görünür → riskli müşteriler listesine gider).
