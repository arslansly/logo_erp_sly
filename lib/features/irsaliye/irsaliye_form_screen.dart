...
/// İrsaliye ekleme/düzenleme ekranı.
/// - `taslakId == null` → yeni irsaliye (önce tür seçimi)
/// - `taslakId != null` → mevcut taslağı düzenle
/// - `onceSecilenCari` dolu → cari önceden set edilir
/// Ek olarak: kaynak/hedef ambar (L_CAPIWHOUSE) + taşıyıcı kodu.
class IrsaliyeFormScreen extends StatefulWidget {
  final int? taslakId;
  final Cari? onceSecilenCari;
  const IrsaliyeFormScreen({super.key, this.taslakId, this.onceSecilenCari});

  @override
  State<IrsaliyeFormScreen> createState() => _IrsaliyeFormScreenState();
}

class _IrsaliyeFormScreenState extends State<IrsaliyeFormScreen> {
  ...
  // Ambar + taşıyıcı lookup
  List<LookupItem> _warehouses = [];
  List<LookupItem> _carriers = [];
  LookupItem? _selectedSource; // sadece kaynak ambar (satış irsaliyesi çıkışı)
  LookupItem? _selectedCarrier;
  ...
  Future<void> _loadLookups() async {
    ...
    try {
      ...
      setState(() {
        ...
        _carriers = results[1];
        _selectedSource = _findById(_warehouses, _selectedSource?.id);
        // Taşıyıcı koda göre eşleştir (taslak yüklenince code ile gelir)
        _selectedCarrier = _findByCode(_carriers, _selectedCarrier?.code);
        _loadingLookups = false;
      });
    } catch (e) {
      debugPrint('[irsaliye_form] Lookup yüklenemedi: $e');
      if (!mounted) return;
      setState(() {
  ...
  LookupItem? _findByCode(List<LookupItem> list, String? code) {
  ...
  Future<void> _loadExistingDraft() async {
    ...
    try {
      final t = await irsaliyeTaslakService.getTaslakById(widget.taslakId!);
      if (!mounted) return;
      setState(() {
        ...
        _selectedSource = t.sourceIndex != null
            ? LookupItem(id: t.sourceIndex!, code: '', name: '')
            : null;
        // Taşıyıcı code ile saklanır; lookup gelince _loadLookups eşleştirir
        _selectedCarrier = (t.shipInfoCode != null && t.shipInfoCode!.isNotEmpty)
            ? LookupItem(id: 0, code: t.shipInfoCode!, name: '')
            : null;
        _selectedCari = Cari(
          id: t.clientRef,
          code: t.clientCode,
  ...
  Widget _buildForm() {
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            Text(widget.taslakId != null ? 'Taslak Düzenle' : 'Yeni İrsaliye'),
        ...
        child: ListView(
          ...
          children: [
            ...
            _AmbarSevkiyatTile(
              ...
              onCarrierChanged: (v) => setState(() => _selectedCarrier = v),
            ),
          ],
        ),
      ),
  ...
  void _oncekleVePaylas() {
    ...
    if (_selectedSource != null) {
      ek['Ambar'] = _selectedSource!.code.isEmpty
          ? _selectedSource!.name
          : '${_selectedSource!.code} — ${_selectedSource!.name}';
    }
    if (_selectedCarrier != null) {
      ek['Taşıyıcı'] = _selectedCarrier!.code.isEmpty
          ? _selectedCarrier!.name
          : '${_selectedCarrier!.code} — ${_selectedCarrier!.name}';
    }
    final data = BelgePdfData(
      ...
      ekBilgiler: ek,
      dosyaAdi: 'TaslakIrsaliye_${_existingDraftId ?? ''}',
    );
    Navigator.push(
      context,
    ...
    try {
      final rate =
          double.tryParse(_exchangeRateController.text.replaceAll(',', '.')) ??
              1.0;
      final draft = IrsaliyeTaslakModel(
        id: _existingDraftId,
        trCode: _tur!.trCode,
        ...
        sourceIndex: _selectedSource?.id,
        destIndex: null, // satış irsaliyesi: sadece kaynak ambar (transfer fişi değil)
        shipInfoCode: _selectedCarrier?.code,
      ...
      final saved = _existingDraftId == null
          ? await irsaliyeTaslakService.createTaslak(draft)
          : await irsaliyeTaslakService.updateTaslak(_existingDraftId!, draft);
      ...
      try {
        final newId = await irsaliyeTaslakService.transferTaslak(saved.id!);
        if (!mounted) return;
        messenger.showSnackBar(SnackBar(
          backgroundColor: AppColors.positive,
  ...
  // Sadece SATIŞ irsaliyelerinde (TRCODE 7/8) stok kontrolü — fiziksel çıkış.
  Future<bool> _confirmStockIfNeeded() async {
    if (_tur == null) return true;
    final satisTrCode = _tur!.trCode == 7 || _tur!.trCode == 8;
...
// ─── Tür seçim ekranı (kategorilere göre gruplu) ───────────────────────────
class _TurSecimEkrani extends StatelessWidget {
  final ValueChanged<IrsaliyeTuru> onSelected;
  const _TurSecimEkrani({required this.onSelected});

  @override
  Widget build(BuildContext context) {
    final gruplar = <String, List<IrsaliyeTuru>>{};
    ...
    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(
        title: const Text('İrsaliye Türü Seç'),
        backgroundColor: AppColors.surface,
        elevation: 0,
        titleTextStyle: AppTypography.h2,
        foregroundColor: AppColors.slate900,
      ...
      body: ListView(
        ...
        children: [
          for (final entry in gruplar.entries) ...[
            Padding(
              padding:
                  const EdgeInsets.fromLTRB(4, AppSpacing.md, 4, AppSpacing.sm),
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                                child:
                                    Text(t.adi, style: AppTypography.body)),
...
// ─── Ambar & Taşıyıcı (kaynak ambar + taşıyıcı) ────────────────────────────
class _AmbarSevkiyatTile extends StatelessWidget {
  final List<LookupItem> warehouses;
  final List<LookupItem> carriers;
  final LookupItem? selectedSource;
  final LookupItem? selectedCarrier;
  final bool loading;
  final String? error;
  final VoidCallback onRetry;
  final ValueChanged<LookupItem?> onSourceChanged;
  final ValueChanged<LookupItem?> onCarrierChanged;

  const _AmbarSevkiyatTile({
  ...
  @override
  Widget build(BuildContext context) {
    final hasAny = selectedSource != null || selectedCarrier != null;
    return Container(
      decoration: BoxDecoration(
        color: AppColors.surface,
        borderRadius: BorderRadius.circular(12),
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          title: Text('Ambar & Taşıyıcı (opsiyonel)', style: AppTypography.h3),
          subtitle: Text(
            _subtitle(),
            style: AppTypography.caption.copyWith(
          ...
          children: [
            if (loading)
              ...
              _lookupDropdown(
                ...
                emptyHint: 'LOGO\'da tanımlı ambar bulunamadı',
              ),
              const SizedBox(height: AppSpacing.sm),
              _lookupDropdown(
                label: 'Taşıyıcı',
                ...
                emptyHint: 'LOGO\'da tanımlı taşıyıcı bulunamadı',
                matchByCode: true,
              ),
            ],
          ],
  ...
  String _subtitle() {
    ...
    return 'Kaynak ambar ve taşıyıcı';
  }
  ...
  Widget _lookupDropdown({
    ...
    // Seçili değeri listedeki gerçek instance ile eşle (id ya da code üzerinden)
    LookupItem? effectiveValue;
    if (value != null) {
      for (final i in items) {
        if (matchByCode ? i.code == value.code : i.id == value.id) {
          effectiveValue = i;
          break;
        }
      }
...
class _LineDraft {
  ...
  factory _LineDraft.fromTaslak(IrsaliyeTaslakSatirModel l) => _LineDraft(
        id: l.id,
        lineType: l.lineType,
        stockRef: l.stockRef,
  ...
  IrsaliyeTaslakSatirModel toModel(bool kdvDahil) => IrsaliyeTaslakSatirModel(
        id: id,
        lineType: lineType,
        stockRef: stockRef,
        stockCode: stockCode,
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Text('Satır toplamı:', style: AppTypography.caption),
                const Spacer(),
