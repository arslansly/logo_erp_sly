...
/// Fatura ekleme/düzenleme ekranı.
/// - `taslakId == null` → yeni fatura (önce tür seçimi)
/// - `taslakId != null` → mevcut taslağı düzenle
/// - `onceSecilenCari` dolu → cari önceden set edilir (cari detayından "Fatura Kes")
class FaturaFormScreen extends StatefulWidget {
  final int? taslakId;
  final Cari? onceSecilenCari;
  // Yapay zeka asistanından gelen ön-dolu satırlar (malzeme + miktar + opsiyonel fiyat).
  final List<AiLineSeed>? onceSecilenSatirlar;
  // AI'dan gelen fatura türü (LOGO TRCODE). Verilmezse Toptan Satış varsayılır.
  final int? onceSecilenTrCode;
  const FaturaFormScreen({
    super.key,
    this.taslakId,
  ...
  @override
  State<FaturaFormScreen> createState() => _FaturaFormScreenState();
}

class _FaturaFormScreenState extends State<FaturaFormScreen> {
  ...
  // backend'e gönderilirken her zaman KDV hariç'e çevrilir (LOGO INVOICE.PRICE).
  ...
  List<CurrencyModel> _currencies = [CurrencyModel.tl]; // TL hep listenin başında
  bool _loadingExchangeRate = false;

  // Ek bilgi alanları (sadece LOGO'dan güvenli çekilenler)
  ...
  void initState() {
    ...
    // AI'dan satır geldiyse: tür seçimini atla (AI'ın türü, yoksa Toptan Satış) ve satırları doldur.
    if (widget.onceSecilenSatirlar != null &&
        widget.onceSecilenSatirlar!.isNotEmpty) {
      _tur = (widget.onceSecilenTrCode != null
              ? FaturaTuru.fromTrCode(widget.onceSecilenTrCode!)
              : null) ??
          FaturaTuru.toptanSatis;
      _seedAiLines();
    }
  ...
  Future<void> _onCurrencyChanged(int? newType) async {
    ...
    try {
      ...
      // Kur alınamadı, kullanıcı manuel girer
      if (!mounted) return;
      _exchangeRateController.text = '';
    } finally {
  ...
  Future<void> _loadExistingDraft() async {
    ...
    try {
      final t = await faturaTaslakService.getTaslakById(widget.taslakId!);
      if (!mounted) return;
      setState(() {
        _existingDraftId = t.id;
        _tur = FaturaTuru.fromTrCode(t.trCode);
        _date = t.date;
        ...
        // Ek alanlar — sadece ID set et, isim listelere geldiğinde _loadLookups doldurur
        _selectedSalesman =
            t.salesmanRef != null ? LookupItem(id: t.salesmanRef!, code: '', name: '') : null;
        _selectedPayDef =
            t.payDefRef != null ? LookupItem(id: t.payDefRef!, code: '', name: '') : null;
        _selectedCari = Cari(
          id: t.clientRef,
          code: t.clientCode,
          title: t.clientTitle,
  ...
  Widget _buildForm() {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.taslakId != null ? 'Taslak Düzenle' : 'Yeni Fatura'),
  ...
  Future<void> _pickMalzemeForLine(int index) async {
    ...
    // Stok + tanımlı fiyat için detayı arka planda çek
    try {
      ...
      setState(() {
        ...
        l.gercekStok = detay.gercekStok;
        // Fiyat boşsa türe göre tanımlı fiyatı koy. Override edebilir.
        final isSatis = _tur?.kategori == 'Satış';
        final tanimli = isSatis ? detay.satisFiyati : detay.satinalmaFiyati;
  ...
  // (1 + KDV/100) ile çarpılır/bölünür — böylece satır toplamı aynı kalır.
  ...
  Future<void> _save({required bool transfer}) async {
    ...
    try {
      final rate = double.tryParse(
          ...
          1.0;
      final draft = FaturaTaslakModel(
        id: _existingDraftId,
        trCode: _tur!.trCode,
        ficheNo: _ficheNoController.text.trim().isEmpty
            ? null : _ficheNoController.text.trim(),
        docCode: _docCodeController.text.trim().isEmpty
            ? null : _docCodeController.text.trim(),
        date: _date,
        ...
        genExp1: _genExp1Controller.text.trim().isEmpty
            ? null : _genExp1Controller.text.trim(),
        lines: _lines.map((l) => l.toModel(_kdvDahil)).toList(),
      );
      draft.recomputeTotals();

      final saved = _existingDraftId == null
          ? await faturaTaslakService.createTaslak(draft)
          : await faturaTaslakService.updateTaslak(_existingDraftId!, draft);
      ...
      // Transfer iste
      try {
        final newId = await faturaTaslakService.transferTaslak(saved.id!);
        if (!mounted) return;
        messenger.showSnackBar(SnackBar(
          backgroundColor: AppColors.positive,
  ...
  // Sadece SATIŞ fatura türlerinde (TRCODE 37/38) stok kontrolü.
  // Stok yetersizse onay dialog'u — kullanıcı isterse devam edebilir
  // (LOGO firma ayarı "negatif stok yasak" ileride config'ten okunacak).
  Future<bool> _confirmStockIfNeeded() async {
    if (_tur == null) return true;
    // Sadece gerçek satış faturalarında kontrol (iade/hizmet/proforma'da değil)
    final satisTrCode = _tur!.trCode == 37 || _tur!.trCode == 38;
    if (!satisTrCode) return true;

    final yetersizler = <String>[];
    for (var i = 0; i < _lines.length; i++) {
      final l = _lines[i];
      if (l.stockRef == null || l.gercekStok == null) continue;
      if (l.amount > l.gercekStok!) {
        yetersizler.add(
          '${i + 1}. ${l.stockName ?? "(malzeme)"} — '
          'stokta ${_fmtPlain(l.gercekStok!)}, isteniyor ${_fmtPlain(l.amount)}',
        );
      }
    }
    if (yetersizler.isEmpty) return true;
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              'Aşağıdaki satırlarda stok yetersiz:',
              style: TextStyle(fontWeight: FontWeight.w600),
            ),
            const SizedBox(height: 8),
            ...yetersizler.map((y) => Padding(
            ...
            const Text(
              'LOGO\'nun firma ayarında "negatif stok yasak" işaretliyse '
              'aktarım yine de hata verebilir.',
...
// ─── Tür seçim ekranı (yeni fatura akışının ilk adımı) ─────────────────────
class _TurSecimEkrani extends StatelessWidget {
  final ValueChanged<FaturaTuru> onSelected;
  const _TurSecimEkrani({required this.onSelected});

  @override
  Widget build(BuildContext context) {
    final gruplar = <String, List<FaturaTuru>>{};
    for (final t in FaturaTuru.values) {
      gruplar.putIfAbsent(t.kategori, () => []).add(t);
    }

    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(
        title: const Text('Fatura Türü Seç'),
        backgroundColor: AppColors.surface,
        elevation: 0,
        titleTextStyle: AppTypography.h2,
        foregroundColor: AppColors.slate900,
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(child: Text(t.adi, style: AppTypography.body)),
...
// ─── Tarih + fiş no + belge no satırı ──────────────────────────────────────
class _DateAndDocRow extends StatelessWidget {
  final DateTime date;
  final ValueChanged<DateTime> onDateChange;
  final TextEditingController ficheNo;
  final TextEditingController docCode;

  const _DateAndDocRow({
    required this.date,
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Row(
          children: [
            Expanded(child: _LabeledField(controller: ficheNo, label: 'Fiş no (opsiyonel)')),
            const SizedBox(width: AppSpacing.sm),
            Expanded(child: _LabeledField(controller: docCode, label: 'Belge no')),
          ],
        ),
      ],
    );
...
class _ExtraFieldsTile extends StatelessWidget {
  ...
  Widget _lookupDropdown({
    ...
    // value listede yoksa (sadece id ile gelen durum) — null göster
...
class _LineDraft {
  ...
  String? stockName;
  String? uomCode;
  double? gercekStok;  // malzeme detayından gelir, satış'ta validasyon için
  ...
  factory _LineDraft.fromTaslak(FaturaTaslakSatirModel l) => _LineDraft(
        id: l.id,
        lineType: l.lineType,
        stockRef: l.stockRef,
  ...
  void setMalzeme(Malzeme m) {
    ...
    stockName = m.ad;
    uomCode = m.birim;
    gercekStok = m.gercekStok;  // detay gelene kadar liste değeri
  }

  double get amount => double.tryParse(amountC.text.replaceAll(',', '.')) ?? 0;
  // Kullanıcının ekrana girdiği ham fiyat (mod'a göre KDV dahil veya hariç)
  double get price => double.tryParse(priceC.text.replaceAll(',', '.')) ?? 0;
  double get vatRate => double.tryParse(vatC.text.replaceAll(',', '.')) ?? 0;
  double get discount => double.tryParse(discountC.text.replaceAll(',', '.')) ?? 0;

  // KDV hariç birim fiyat — backend'e ve hesaplara hep bu gider
  ...
  FaturaTaslakSatirModel toModel(bool kdvDahil) => FaturaTaslakSatirModel(
        id: id,
        lineType: lineType,
        stockRef: stockRef,
        stockCode: stockCode,
        ...
        lineDescription: aciklama.text.trim().isEmpty ? null : aciklama.text.trim(),
      );
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Expanded(
                    child: _toplamMini('Brüt', brut),
                  ),
                  Expanded(
                    child: _toplamMini('İskonto', iskonto, neg: true),
                  ),
                  Expanded(
                    child: _toplamMini('KDV', kdv),
                  ),
                  Expanded(
                    child: _toplamMini('Net', net, bold: true),
                  ),
                ],
              ),
              const SizedBox(height: AppSpacing.sm),
                ...
                children: [
                  ...
                  Expanded(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(isSaving ? 'Kaydediliyor...' : 'Kaydet ve Aktar'),
